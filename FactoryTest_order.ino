...
int count_flag = 0, rx_num = 0, rx_count = 0;
...
void encoder(){
  static int32_t count = 65536,count_last = 65536,count_change = 0;
  static uint8_t code = 0,code_old = 0;
  if(!setup_flag){
    setup_flag = 1;
    gpio_reset_pin(GPIO_NUM_2);
...
void rfid(){
  ...
  if(error==0){
    ...
    M5.Lcd.print("Please put the card        ");
    ...
    M5.Lcd.setCursor(0, 150, 4);
    M5.Lcd.print("Please connect to devices");
  }
}
